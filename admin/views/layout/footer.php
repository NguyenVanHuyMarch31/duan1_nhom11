...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        const rowsPerPage = 5; // Số tin tức trên mỗi trang
        const table = document.querySelector("#newsTable"); // Bảng tin tức
        const rows = table.querySelectorAll(".newsRow"); // Lấy tất cả các hàng tin tức
        const totalRows = rows.length; // Tổng số hàng
        const totalPages = Math.ceil(totalRows / rowsPerPage); // Tính tổng số trang
        const pagination = document.querySelector(".paginationNav ul"); // Nơi hiển thị các nút phân trang

        let currentPage = 1; // Mặc định trang hiện tại là trang 1
        ...
        function showPage(page) {
            ...
            rows.forEach((row, index) => {
                // Ẩn các hàng không thuộc trang này
