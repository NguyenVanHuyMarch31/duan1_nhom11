...
class quanTriPhimModel
{
    ...
    public function updateMovie($movie_id, $movie_name, $genre_id, $duration, $description, $director, $actors, $release_date, $trailer, $poster_name) {
        $sql = "UPDATE movie SET movie_name = ?, duration = ?, description = ?, director = ?, actors = ?, release_date = ?, trailer = ?, poster = ? WHERE movie_id = ?";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([$movie_name, $duration, $description, $director, $actors, $release_date, $trailer, $poster_name, $movie_id]);
    }
